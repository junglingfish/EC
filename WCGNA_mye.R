...
seuratobj.data = readRDS('mye_des_labeled.rds')


#WGCNA
cluster <- seuratobj.data$seurat_clusters
idx <- c(which(cluster==0), which(cluster==1), which(cluster==3), which(cluster==5), which(cluster==6), which(cluster==7))

# idx <- c(which(cluster==1), which(cluster==5))

seuratobj.data <- seuratobj.data[,idx]
#select cells
x <- 1:length(colnames(seuratobj.data@assays$RNA@counts))
id <- sample(x, size = 80, replace = F)
...
moduleTraitCor <- moduleTraitCor[order(moduleTraitCor[,7], decreasing = T),]
# id <- which(moduleTraitCor[,7] < -0.13 | moduleTraitCor[,7] > 0.13)
# id <- id[-2]
# moduleTraitCor <- moduleTraitCor[id,-7]
moduleTraitCor <- moduleTraitCor[,-7]
textMatrix <- cbind(textMatrix, sta)
textMatrix <- textMatrix[order(textMatrix[,7], decreasing = T),]
textMatrix <- textMatrix[,-7]
...
setwd("D:/ZJU-FISH/BI/EC/data/GSE160269/mye_new_genes/")
#gene
...
cluster1 = as.data.frame(design[,6])
names(cluster1) = "cluster7"
...
write.csv(data, "mye_gene_c7_salmon.csv")
