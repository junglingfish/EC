...
sce <- read.table(file="UMI_matrix_Tcell.txt")
#head(sce)
...
cluster2celltype <- c("0"="0",
                      ...
                      "5"= "2",
                      "6"= "1")
cd8[['cell_type']] = unname(cluster2celltype[cd8@meta.data$seurat_clusters])
DimPlot(cd8, reduction = 'umap', group.by = 'cell_type', label = TRUE, pt.size = 0.5, label.size = 6, raster=FALSE)

new.cluster.ids <- c("C0", "C1", "C2", "C3", "C4", "C5", "C6")
# new.cluster.ids <- c("C0", "C1", "C1", "C1", "C0", "C2", "C1")
...
#
features_escc <- c('CD69', 'ITGAE', 'ITGA1', 'CCR7', 'SELL', 'S1PR1')
DotPlot(cd8, features = features_escc, group.by = 'cell_type') + RotatedAxis()

features_escc <- c('IFNG', 'GZMB', 'GZMH', 'PRF1', 'NKG7')
DotPlot(cd8, features = features_escc, group.by = 'cell_type') + RotatedAxis()

FeaturePlot(cd8, features = c('IFNG', 'TNFA', 'GZMB', 'GZMH', 'GZMK', 'PRF1', 'NKG7'), group.by = 'cell_type', min.cutoff = "q9")
...
cluster2celltype <- c("0"="0",
                      ...
                      "5"= "2",
                      "6"= "1")
mysce[['cell_type']] = unname(cluster2celltype[mysce@meta.data$seurat_clusters])

saveRDS(mysce, file = 'cd8_modify221218.rds')
...
markers <- c('IFNG', 'GZMB', 'GZMH', 'PRF1', 'NKG7')  
my36colors <-c('#E5D2DD', '#53A85F', '#F1BB72', '#F3B1A0', '#D6E7A3', '#57C3F3', '#476D87', '#E95C59', '#E59CC4', '#AB3282', '#23452F', '#BD956A', '#8C549C', '#585658', '#9FA3A8', '#E0D4CA', '#5F3D69', '#C5DEBA', '#58A4C3', '#E4C755', '#F7F398', '#AA9A59', '#E63863', '#E39A35', '#C1E6F3', '#6778AE', '#91D0BE', '#B53E2B', '#712820', '#DCC1DD', '#CCE0F5',  '#CCC9E6', '#625D9E', '#68A180', '#3A6963', '#968175')#颜色设置  
VlnPlot(cd8, features = markers, stacked=T,pt.size=0, cols = my36colors, group.by = 'cell_type', direction = "horizontal", x.lab = '', y.lab = '')+theme(axis.text.x = element_blank(),  axis.ticks.x = element_blank())#不显示坐标刻度
...
select_obj <- FindClusters(select_obj, resolution = 0.15)

saveRDS(select_obj, file = 'special_genes_4clusters.rds')
# #12
...
new.cluster.ids <- c("C0", "C1", "C0", "C2")
# new.cluster.ids <- c("C0", "C1", "C1", "C1", "C0", "C2", "C1")
names(new.cluster.ids) <- levels(select_obj)
select_obj <- RenameIdents(select_obj, new.cluster.ids)
DimPlot(select_obj, reduction = "umap", label = TRUE, pt.size = 0.5, label.size = 6, raster=FALSE)

FeaturePlot(select_obj, features = c('CD3D', 'CD3E', 'CD8A', 'CD69',  'ITGAE',  'ITGA1', 'ENTPD1', 'PDCD1' ,'CTLA4','HAVCR2'), min.cutoff = "q10")

markers <- FindAllMarkers(select_obj, only.pos = TRUE)
write.table(markers, file = 'special_genes_merged.csv', sep = ',')

y = Idents(select_obj)
write.table(y, file = "spe_merged.csv", sep = ',')

saveRDS(select_obj, file = 'special_genes_3clusters.rds')

library(loomR)
library(SeuratDisk)
spe = readRDS('special_genes_3clusters.rds')
setwd("D:/ZJU-FISH/BI/EC/SCENIC/")
spe.loom <- as.loom(select_obj, filename = "sceloom_spee.loom", verbose = FALSE)

library(SCopeLoomR)
loom <- open_loom("D:/ZJU-FISH/BI/EC/SCENIC/sceloom_spee.loom")
# loom <- connect("~/sceloom_spee.loom",mode = 'r')

sp <- readRDS('special_genes.rds')
sp.loom <- as.loom(sp, filename = "sceloom_delete.loom", verbose = FALSE)

loom <- open_loom("~/sceloom_delete.loom")

#
features_escc <- c('CD101', 'CD69', 'ITGAE', 'ITGA1', 'PDCD1', 'CTLA4', 'ENTPD1', 'MKI67')
DotPlot(select_obj, features = features_escc) + RotatedAxis()

#
features_escc <- c('CD3D', 'CD3E',  'CD8A', 'ENTPD1', 'CD69', 'ITGAE',    'ITGA1', 'PDCD1', 'HAVCR2', 'TIGIT')
DotPlot(select_obj, features = features_escc) + RotatedAxis()
...
library(MySeuratWrappers)  
#需要展示的基因  
markers <- c('LAG3', 'TIGIT', 'PDCD1', 'HAVCR2', 'CTLA4', 'LAYN')  
my36colors <-c('#E5D2DD', '#53A85F', '#F1BB72', '#F3B1A0', '#D6E7A3', '#57C3F3', '#476D87', '#E95C59', '#E59CC4', '#AB3282', '#23452F', '#BD956A', '#8C549C', '#585658', '#9FA3A8', '#E0D4CA', '#5F3D69', '#C5DEBA', '#58A4C3', '#E4C755', '#F7F398', '#AA9A59', '#E63863', '#E39A35', '#C1E6F3', '#6778AE', '#91D0BE', '#B53E2B', '#712820', '#DCC1DD', '#CCE0F5',  '#CCC9E6', '#625D9E', '#68A180', '#3A6963', '#968175')#颜色设置  
