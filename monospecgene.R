...
cd8 <- readRDS("special_genes_3clusters.rds")
# #merge
...
# expr_matrix <- as(as.matrix(cd8@assays$RNA@counts), 'sparseMatrix')
##提取表型信息--细胞信息(建议载入细胞的聚类或者细胞类型鉴定信息、实验条件等信息)
expr_matrix <- as(as.matrix(cd8@assays$RNA@counts), 'sparseMatrix')
##提取表型信息到p_data(phenotype_data)里面 
p_data <- cd8@meta.data 
p_data$celltype <- cd8@active.ident  ##整合每个细胞的细胞鉴定信息到p_data里面。如果已经添加则不必重复添加
##提取基因信息 如生物类型、gc含量等
f_data <- data.frame(gene_short_name = row.names(cd8),row.names = row.names(cd8))
...
# ##使用seurat选择的高变基因???
# express_genes <- VariableFeatures(cd8)
# cds <- setOrderingFilter(cds, express_genes)
# plot_ordering_genes(cds)
# # 
# ##使用clusters差异表达基因
# deg.cluster <- FindAllMarkers(cd8)
...
setwd("D:/ZJU-FISH/BI/EC/data/monocle")

pdf("specgene.pseudotime.pdf",width = 10,height = 7)
plot_cell_trajectory(cds, color_by="Pseudotime", size=1, show_tree = TRUE, show_backbone=TRUE) 
dev.off()

pdf("specgene.state.pdf",width = 10,height = 7)
plot_cell_trajectory(cds, color_by="State", size=1, show_tree = TRUE, show_backbone=TRUE) 
dev.off()

pdf("specgene.celltype.pdf",width = 10,height = 7)
plot_cell_trajectory(cds, color_by=Idents(cd8), size=1, show_tree = TRUE, show_backbone=TRUE) + scale_color_manual(values=c("#ff3333", "#ffc5b2", "#34bb4f", "#336cb1", "#d3ff33")) + theme(legend.position = "right")
dev.off()
# pdf("cd8.state.pdf",width = 15,height = 7)
# plot_cell_trajectory(cds, color_by="State", size=1, show_tree = TRUE, show_backbone=TRUE) + scale_color_manual(values=c("darkblue", "darkred", "orange", "green", "yellow"))
# dev.off()
# 
# pdf("cd8.celltype.pdf",width = 15,height = 7)
# plot_cell_trajectory(cds, color_by=Idents(cd8), size=1, show_tree = TRUE, show_backbone=TRUE) + scale_color_manual(values=c("darkblue", "darkred", "orange", "green", "yellow", "purple"))
# dev.off()