...
seuratobj.data <- readRDS("cd8_labeled.rds")

new.cluster.ids <- c("C0", "C1", "C1", "C1", "C0", "C2", "C1")
names(new.cluster.ids) <- levels(seuratobj.data)
seuratobj.data <- RenameIdents(seuratobj.data, new.cluster.ids)

# Select the clusters want to map
cluster = Idents(seuratobj.data)
idx_cluster <- c(which(cluster=="C0"), which(cluster=="C1"), which(cluster=="C2"))
...
markers.to.plot <- c("cyto", "exha", 'naive', 'trm')
tiff('cd8_score.tiff', units="in", width=8, height=6.5, res=300, compression = 'lzw')
DotPlot(seuratobj.sel, features = rev(markers.to.plot), cols = c('blue','red'), dot.scale = 16) + RotatedAxis() +  ggtitle("CD8+ T cell cycle")
dev.off()
...
seuratobj.data <- readRDS("cd8_sampletype.rds")

# Select the clusters want to map
sampletype = seuratobj.data$sampletype
idx_sample <- c(which(sampletype=='Normal'))
seuratobj.sel <- seuratobj.data[,idx_sample]
...
tiff('cd8_circle_normal.tiff', units="in", width=8, height=6, res=300, compression = 'lzw')
DotPlot(seuratobj.sel, features = rev(markers.to.plot), cols = c("blue", "red"), dot.scale = 16) + RotatedAxis() +  ggtitle("CD8+ T cell cycle")
dev.off()
