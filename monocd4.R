...
cd4 <- readRDS("cd4.rds")
# expr_matrix <- as(as.matrix(cd4@assays$RNA@counts), 'sparseMatrix')
##提取表型信息--细胞信息(建议载入细胞的聚类或者细胞类型鉴定信息、实验条件等信息)
expr_matrix <- as(as.matrix(cd4@assays$RNA@counts), 'sparseMatrix')
##提取表型信息到p_data(phenotype_data)里面 
p_data <- cd4@meta.data 
p_data$celltype <- cd4@active.ident  ##整合每个细胞的细胞鉴定信息到p_data里面。如果已经添加则不必重复添加
##提取基因信息 如生物类型、gc含量等
f_data <- data.frame(gene_short_name = row.names(cd4),row.names = row.names(cd4))
...
# 
##使用clusters差异表达基因
deg.cluster <- FindAllMarkers(cd4)
...
pdf("cd4.pseudotime.pdf",width = 7,height = 7)
plot_cell_trajectory(cds,color_by="Pseudotime", size=1, show_backbone=TRUE) 
dev.off()

pdf("cd4.state.pdf",width = 7,height = 7)
plot_cell_trajectory(cds,color_by="State", size=1, show_backbone=TRUE)
dev.off()

pdf("cd4.celltype.pdf",width = 7,height = 7)
plot_cell_trajectory(cds, color_by=Idents(cd4), size=1, show_tree = TRUE, show_backbone=TRUE) + scale_color_manual(values=c("darkblue", "darkred", "orange", "green", "yellow", "purple"))
dev.off()
