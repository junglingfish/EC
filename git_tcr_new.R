...
for (i in 1:length(all)){
  ...
  filepath = paste0("D:/ZJU-FISH/BI/EC/data/data_ZJU/data/filtered_contig_annotations.P", seq, ".csv")
  ...
  if(fileflag == TRUE){
    ...
    buffer_barcode <- paste0(patientname, "T.I.", buffer_barcode)
    ...
    for (i in 1:length(Cn8)){
      ...
      flag <- paste0(patient, '_CD8_C', Cn4[i])
    ...
    buffer8 <- append(buffer8, paste0(patientname, '_cd8'))
...
saveWorkbook(wb,file = "tcr_all.xlsx")