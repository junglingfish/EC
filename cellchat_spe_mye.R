...
mysce.t <- readRDS("spe_nor_reset.rds")
...
saveRDS(mysce.merge, file = 'cellchat_spe_mye.rds')
...
write.table(df.net, file = 'net_lr_spe_mye.csv', sep = ',')

#Infer the cell-cell communication at a signaling pathway level
cellchat <- computeCommunProbPathway(cellchat)
df.netp <- subsetCommunication(cellchat, slot.name = 'netP')
write.table(df.netp, file = 'net_pathway_spe_mye.csv')
...
# Chord diagram
par(mfrow=c(1,1))
netVisual_aggregate(cellchat, signaling = pathway.show, sources.use = c(1:18), targets.use = c(1:18), layout = "chord")
...
netVisual_bubble(cellchat, sources.use = c(1:16), targets.use = c(4), pairLR.use = pairLR.use, remove.isolate = TRUE)
