...
spe <- readRDS(file = 'special_genes_3clusters.rds')
...
cluster2celltype <- c("0"="0",
                      ...
                      "2"="0", 
                      "3"= "2")
spe[['cell_type']] = unname(cluster2celltype[spe@meta.data$seurat_clusters])
...
# i = 1
  ...
  if(fileflag == TRUE){
    ...
    # buffer_barcode <- paste0(patientname, ".I.", buffer_barcode)
    ...
    for (i in 1:length(Cn8)){
      ...
      flag <- paste0(patient, '_N_SPE_C', Cn8[i])
    ...
    buffer8 <- append(buffer8, paste0(patientname, '_N_spe'))
    # buffer8 <- append(buffer8, paste0(patientname, '_spe'))
...
saveWorkbook(wb,file = "tcr_spe_N_merged.xlsx")