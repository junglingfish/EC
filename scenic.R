...
sceraw = readRDS('special_genes.rds')
sce <- as.SingleCellExperiment(sce)
setwd("D:/ZJU-FISH/BI/EC/SCENIC/")
sce <- as.loom(sceraw, filename = "sceloom_spe1.loom", verbose = FALSE)
...
loom <- open_loom('D:/ZJU-FISH/BI/EC/SCENIC/sceloom_spe1.loom')
...
#cellInfo
setwd("D:/ZJU-FISH/BI/EC/data")
spe <- readRDS(file = 'special_genes_labeled')
cellInfo <- data.frame(seuratCluster=Idents(spe))
...
#loom
loom <- open_loom('D:/ZJU-FISH/BI/EC/SCENIC/sceloom_spe_merged.loom')
exprMat <- get_dgem(loom)
cellInfo <- get_cell_annotation(loom)
close_loom(loom)
#
...
setwd("D:/ZJU-FISH/BI/EC/data/")
pbmc <- readRDS('special_genes_3clusters.rds')
current.cluster.ids <- c(0:2)
new.cluster.ids <- c("C0", "C1", "C2")
